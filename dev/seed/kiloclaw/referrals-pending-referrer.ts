...
const SCENARIO = 'referrals-pending-referrer';
...
export async function run(): Promise<void> {
  ...
  await insertSeedUsers([
    {
      id: referrerUserId,
      email: referrerEmail,
      name: 'Seed Pending Referrer',
    },
    {
      id: refereeUserId,
      email: refereeEmail,
      name: 'Seed Pending Referee',
    },
  ]);
  await assertUserCount({ userIds, expectedCount: 2 });

  console.log(
    `[${SEED_SCOPE}] Inserting a trial subscription for the referrer and an active subscription for the referee`
  );
  const { subscription: referrerTrialSubscription } = await insertPersonalSubscription({
    userId: referrerUserId,
    sandboxId: `sandbox-${referrerUserId}`,
    name: 'Seed Pending Referrer Trial',
    plan: 'trial',
    status: 'trialing',
  ...
  const { subscription: refereeSubscription } = await insertPersonalSubscription({
    ...
    name: 'Seed Pending Referee Active',
    plan: 'standard',
    status: 'active',
  ...
  const [referralTouch] = await db
    ...
    .values({
      ...
      tracking_value_length: 44,
      is_tracking_value_accepted: true,
      rs_code: opaqueReferralIdentifier,
      rs_share_medium: 'email',
      rs_engagement_medium: 'share',
  ...
  console.log(
    `[${SEED_SCOPE}] Materializing the qualified referral with a pending referrer reward`
  );
    ...
    .values([
      {
        ...
        applied_at: '2026-04-15T16:41:00.000Z',
      },
      {
        conversion_id: conversion.id,
        ...
        earned_at: convertedAt,
        expires_at: addMonthsUtc(convertedAt, 12),
      },
    ])
    .returning({
  ...
  await db.insert(credit_transactions).values({
    ...
    is_free: false,
    description: 'Seed pending-referrer paid period',
